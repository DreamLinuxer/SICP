...
(define (install-polynomial-package)
  ...
  (define (pseudoremainder-terms L1 L2)
    (let ((t1 (first-term L1))
	  (t2 (first-term L2)))
      (let ((factor (exp (coeff t2) (+ 1 (- (order t1) (order t2))))))
	(cadr (div-terms (mul-term-by-all-terms (make-term 0 factor) L1) L2)))))
  
  (define (remainder-terms a b)
    (cadr (div-terms a b)))

  (define (find-common-factor L)
    (if (empty-termlist? (rest-terms L))
	(coeff (first-term L))
	(greatest-common-divisor (coeff (first-term L))
				 (find-common-factor (rest-terms L)))))
  
  (define (remove-common-factor factor L)
    (car (div-terms L (adjoin-term (make-term 0 factor)
				   (the-empty-termlist L)))))

  (define (gcd-terms a b)
    (if (empty-termlist? b)
	(remove-common-factor (find-common-factor a) a)
	(gcd-terms b (pseudoremainder-terms a b))))

  (define (reduce-terms a b)
    (let ((divisor (gcd-terms a b)))
      (let ((tg (first-term divisor))
	    (t1 (first-term a))
	    (t2 (first-term b)))
	(let ((factor (exp (coeff tg) (+ 1 (- (if (> (order t1) (order t2))
						  (order t1)
						  (order t2))
					      (order tg))))))
	  (let ((aa (car (div-terms (mul-term-by-all-terms (make-term 0 factor) a) divisor)))
		(bb (car (div-terms (mul-term-by-all-terms (make-term 0 factor) b) divisor))))
	    (let ((cfactor (gcd (find-common-factor aa)
				(find-common-factor bb))))
	      (list (remove-common-factor cfactor aa)
		    (remove-common-factor cfactor bb))))))))
